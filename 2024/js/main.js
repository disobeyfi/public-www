...
var changeColor = function(){
	...
	if(  video.currentTime < 10.5){
		...
		$("#main-mobile-logo").attr("src", BASEURL + "img/logo_mobile_dbh.webp")
		$("#navi-frontpage img").attr("src", BASEURL + "img/just_logo_red.png")
		
		$("#main-logo").attr("src", BASEURL + "img/logo_dbh.webp")
...
var DOinitWaypoints = function(){
	...
	}catch(error){ console.log("No program-highlights on page") }
	...
	}catch(error){ console.log("No cfp on page") }
...
/*-------------------------
 alternate subpage headertext
 -----------------------*/

 var alternateSubpageHeader = function(){

	if( $("body").hasClass("subpage")){

		setTimeout(() => {
			
			if( $("#logo-subpage").hasClass("dbh")  ){
				$("#logo-subpage").attr("src", BASEURL + "img/logo.webp").removeClass("dbh")
			}else{
				$("#logo-subpage").attr("src", BASEURL + "img/logo_dbh.webp").addClass("dbh")
			}
